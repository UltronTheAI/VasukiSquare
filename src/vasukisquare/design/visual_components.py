...
def render_comparison_block(
    ...
    l_list = "".join(f'<li><span class="comp-icon">{l_icon}</span><span>{RichTextRenderer.render_text_or_markdown(item)}</span></li>' for item in left_items)
    r_list = "".join(f'<li><span class="comp-icon">{r_icon}</span><span>{RichTextRenderer.render_text_or_markdown(item)}</span></li>' for item in right_items)
...
def render_timeline_block(
    ...
    for idx, item in enumerate(items):
        time_tag = html.escape(item.get("time") or item.get("step") or f"Phase {idx+1}")
...
def render_exercise_block(
    ...
    obj_html = f'<p class="exercise-objective"><strong>Objective:</strong> {RichTextRenderer.render_text_or_markdown(objective)}</p>'

    instr_items = "".join(f'<li>{RichTextRenderer.render_text_or_markdown(i)}</li>' for i in instructions)
    instr_html = f'<ol class="exercise-instructions">{instr_items}</ol>'
    ...
    hints_html = ""
    if hints:
        h_items = "".join(f'<li>{RichTextRenderer.render_text_or_markdown(h)}</li>' for h in hints)
    ...
    <div class="component-exercise theme-{theme.value}">
      <div class="exercise-header">
        <div class="exercise-header-left">
          ...
          <h4 class="exercise-title">{html.escape(title)}</h4>
        </div>
        {diff_badge}
      </div>
